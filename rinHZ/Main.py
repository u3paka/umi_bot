...
if __name__ == '__main__':
	# twf.tweet(ans = 'testtt', screen_name = 'xxxx', status_id = '', imgfile = '')
	Stream(auth)
	...
	# )
	# })
	# print(datetime.strftime(datetime.utcnow(), '%m月%d日 %H時%M分%S秒') + 'です')

	# user = twtr.get_user(screen_name = '_rinHz')._json
	#時間計測(秒)
	# print(user)
	# imitate(screen_name = 'umi0315_pokemon')
	# defaultProfile()
	# print(os.path.exists('/Users/xxxx'))
	# a = twtr.rate_limit_status()
	# a = [utiltools.cleanText(status._json['text']) for status in twtr.user_timeline(screen_name = 'syokiRincyan', count = 200) if isAllowed(status._json) == True]
