...
def getTweetList(n = 1000):
	try:
		...
		with db.transaction():# and Tweets.text.contains('怒')
			...
			tweetslist = [cleanText(tweet.text) for tweet in tweets]
			return tweetslist
...
# sList = getTweetList(10)
print(sList)
maList = [NLP.MA.getMeCab(s, mode = 7, form = ['名詞', '形容詞','副詞', '動詞'], exception = ["記号"], isDebug = False) for s in sList]
# words はさっきの単語リスト
