...
import skflow
import NLP, dealSQL, utils
# from sklearn.cross_validation import train_test_split
from sklearn import cross_validation
### Training data

def train(DataSet, saveDIR = "/Users/xxxx'):
    ...
    if splittype == 'ma':
        DataSet = [(' '.join([w[0] for w in NLP.MA.getMeCabCP(pair[0])]), pair[1]) for pair in DataSet]
    else:
...
def predictAns(s, model = "/Users/xxxx'):
    MAX_DOCUMENT_LENGTH = 10
    classifier = skflow.TensorFlowEstimator.restore(model)
    vocab_processor = skflow.preprocessing.VocabularyProcessor(MAX_DOCUMENT_LENGTH)
    if splittype == 'ma':
        test = ' '.join([w[0] for w in NLP.MA.getMeCabCP(s)])
    else:
        test= ' '.join(list(s))
    print(test)
...
if __name__ == '__main__':
    ...
    ### Process vocabulary
    CAT1 = [(tweet, 1) for tweet in dealSQL.getTweetList(n = 1000,UserList = [], contains = '')]
    CAT2 = [(tweet, 2) for tweet in dealSQL.getTweetList(n = 10,UserList = [], contains = '')]
...
'''
    ...
    # umi = [(sss, 2) for sss in utils.flatten([ss.split('。') for ss in s.split('\n')]) if sss != '']
    # print(umi)
    # CAT2 =
    DataSet = utils.f7(CAT1 + CAT2)
    # DataSet = [('そのとおり', 1), ('ちがうよ',2),()]
    train(DataSet, saveDIR = "/Users/xxxx/OneDrive/imgs/RNNtxtclass2")
    # s = 'どういうこと'
    # predictAns(s, model = "/Users/xxxx/OneDrive/imgs/RNNtxtclass")
