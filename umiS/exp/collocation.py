...
def getTrigram(startWith = '', Plist = ['<BOS>','名詞','名詞','名詞','助詞','動詞','助詞','助詞','名詞','名詞','名詞','助詞','動詞','助動詞','助動詞','<EOS>'], endWithList = ['。', '!', '！', '?', '？'], isDebug = False, n = 50):
	QuestionPhrase = '<KEY>...？'
	if startWith != '':
		ans = ['<BOS>', startWith]
	else:
...
if __name__ == '__main__':
	...
	tweetslist = getTweetList(10000, [])
	items = [' '.join([w[0] for w in NLP.MA.getMeCabCP(tweet) if w[1] in ['名詞', '動詞', '形容詞', '固有名詞']]) for tweet in tweetslist]
