...
def Haiku(mas):
    ...
    yomi = [ma[8] for ma in mas]
    wlens = [len(w) for w in yomi]
    if not np.sum(wlens) in [17, 18, 19]:
        return ''
    else:
        wlenArr = [np.sum(wlens[0:i+1]) for i in range(len(wlens))]
        if 5 in wlenArr and 12 in wlenArr and (17 in wlenArr or 18 in wlenArr):
            index1 = wlenArr.index(5)+1
            index2 = wlenArr.index(12)+1
        elif 5 in wlenArr and 13 in wlenArr and (18 in wlenArr or 19 in wlenArr):
            index1 = wlenArr.index(5)+1
